...
## **Introduction**
Mental health and heart disease are prominent public health challenges within Scotland@. The recent 2022 census revealed that the percentage of individuals reporting mental health conditions, conditions that affect your well-being, increased from 4.4% in 2011 to a staggering 11.3%, denoting the greatest change across all health categories. Heart disease describes the range of conditions impeding heart activity and structure, with coronary heart disease occurring most frequently. Since the early 2000s, cardiovascular disease mortality has reduced, however records depict a recent incline @. 
The study by @Alot found that individuals with severe mental illness, including schizophrenia, bipolar disorder and sever depression, demonstrated higher risk for cardiovascular disease resulting in premature death. Antidepressants treat multiple mental health conditions, however, the noradrenergic and specific serotonergic antidepressant (NASSA) Mitrazapine is used in the UK as a follow-up treatment if first-line SSRIs fail to work. Therefore, prescription trends can offer valuable insights into depression trends.
This assessment explores the intersection of NASSA usage and heart disease mortality in Scotland by visualizing trends across health boards and age groups. It initially highlights that general antidepressant prescriptions have risen while in-patient mental health admissions reduced. Additionally, that mortality rates are increasing over time, with variations in NASSA usage across health board regions potentially correlating with increased death rates.
...
Data is prepared and tidied, ready for visualising in a combined plot. This line and bar plot demonstrate the trend in in-Patient admission cases and antidepressant usage respectively, from 2015-2023, observe change in antidepressant usage and community-based care.
...
Spatial_Plot <- Mirtazapine_shape %>%
  ...
  theme(panel.grid = element_blank())  # Remove grid lines present
...
### Summary
Map observations, show Mirtazapine prescriptions per person is high in 'NHS Ayrshire and Arran' and low in 'NHS Highland', represented by the viridis colour scale. Other higher usage areas include 'NHS Glasgow and Clyde' and 'NHS Lanarkshire', and lower includes 'NHS Lothian'. As Mirtazapine is often reserved as a last line antidepressant, this indicates that depression levels may be greater in these areas.

## **Table of heart disease differences when Mirtazapine prescription is high versus low**
It is interesting to compare heart disease mortality rates between 'NHS Ayrshire and Arran' with high Mitrazapine prescription per person and 'NHS Highland' with low rates. Both health boards have similar populations, relevant comparison. The accompanying gt table will focus on the 45-64 age group, as they experienced greatest death increase.
...
HD_Table <- HD_Table_Prep %>%
 ...
 cols_align(align = "center", columns = NumberOfDeaths) %>% #center-align the 'Number of Deaths' column
  ...
  tab_options(., container.width = 500, container.height = 500) # for plot display size
